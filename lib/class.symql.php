	...
	Class SymQL {
		...
		*/
		private function indexFieldsByID($fields_list, $section_fields, $return_object = false) {
			if (!is_array($fields_list) && !is_null($fields_list)) {
			...
			foreach ($fields_list as $field) {
				...
				if (!is_numeric($field_name)) {
					$field_name = reset(explode(':', $field_name));
				}
				...
				if ($remove) {
					throw new Exception(sprintf("%s: field '%s' does not exist", __CLASS__, $field));
				}
			}
		...
		public static function run(SymQLQuery $query, $output = SymQL::RETURN_XML) {
			...
			foreach ($filters as $filter) {
				...
				if (!$field->canFilter() || !(method_exists($field, 'buildDSRetrivalSQL') || method_exists($field, 'buildDSRetrievalSQL'))) {
				...
				if (is_string($filter['value'])) {
					...
					$value = $filter['value'];
					if ($field instanceof FieldSelectBox_Link) {
						$filter_type = self::DS_FILTER_AND;
					}
					else {
						$filter_type = self::DS_FILTER_OR;
					}
				}
			...
			switch ($output) {
				...
				case SymQL::RETURN_XML:
					...
					if ($pagination) {
						$result->appendChild(General::buildPaginationElement(
